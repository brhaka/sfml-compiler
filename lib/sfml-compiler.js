	...
	module.exports = SfmlCompiler = {
		...
		config: {
			regularFiles: {
				...
				properties: {
					sameAsMain: {
						title: 'Include resources (.png, .jpg, .ttf, .mp3, .ogg) from the main.cpp directory',
						type: 'boolean',
						"default": true
					},
					copyAll: {
						title: 'Include all folders, subfolders and files from the main.cpp directory',
						type: 'boolean',
						"default": false
					},
					resourcesDir: {
						...
						type: 'string',
						"default": "Leave blank if resources are in the same directory as main.cpp"
					},
					dllsDir: {
						title: 'Directory for DLLs that should be included',
						type: 'string',
						"default": "C:\\SFML\\bin"
					},
					libstdcDir: {
			...
			deleteBat: {
				title: 'Delete compiler.bat when compilation is done',
				type: 'boolean',
				"default": true
			},
			createLog: {
		...
		compile: function () {
			...
			editor = "";
			if(atom && atom.workspace) { editor = atom.workspace.getActivePaneItem(); }
			...
			fs.writeFile(pathToCurrentWorkingDir + "\\compiler.bat", compileCommands, function(e,r) {if (e) console.log("Error: ",e)});
